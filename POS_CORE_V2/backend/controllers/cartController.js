...
exports.processPurchase = async (req, res) => {
  try {
    ...
    if (tipoPago === 'mixto') {
      ...
      if (tipoPago === 'tarjeta' && monto > total) {
        return res.status(400).send('❌ El pago con tarjeta no puede ser mayor al total.');
      }

      if (tipoPago === 'efectivo') pagoEfectivo = monto;
      if (tipoPago === 'tarjeta') pagoTarjeta = monto;
      if (tipoPago === 'transferencia') pagoTransferencia = monto;
    }
